...
;; adding a helper function to extend an environment
  ...
  (cond [(var? e) 
        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if
             (not (and (int? v1)
                        (int? v2)))
             (error "MUPL illegal use of ifgreater")
        ...
        [(call? e)
         ...
         (if
           ...
           (begin
             ([print c]
              [error "MUPL illegal first argument to call"]))
           (let*
               ([fun (closure-fun c)]
                [body (fun-body fun)]
                [name (fun-nameopt fun)])
             (eval-under-env body (env-extend (env-extend
                                               (closure-env c)
                                               (fun-formal fun)
                                               actual) name c)))))]
        ...
        [(fst? e)
         (let ([e (eval-under-env (fst-e e) env)])
           (if (apair? e)
               (apair-e1 e)
               (error "MUPL fst applied on a non-pair argument")))]
